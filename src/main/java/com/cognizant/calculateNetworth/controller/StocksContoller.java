...
public class StocksContoller {
	...
	@Autowired
	private ShareDetailsFiegnProxy shareDetailsFeignproxy;
	...
	@GetMapping("/calculatenetworth/{portfolioId}")
	public double getAsset(@RequestHeader("Authorization") String token,@PathVariable(value = "portfolioId") int portfolioId) 
	{
		...
		List<Asset> assets = assetservice.getAllAssetForPortfolio(portfolioId);
		...
		if (!stockAssetList.isEmpty()) {
			stockValueList = shareDetailsFeignproxy.finddailyShareById(token,stockAssetList);
		}
		if (!mutualFundAssetList.isEmpty()) {
			mutualFundValueList = mutualFundDetailsFeignProxy.getMutualDetailsById(token,mutualFundAssetList);
		}
	...
	public List<Asset> getAllStocks(@RequestHeader("Authorization") String token) {
		...
		List<StockDetails> shareslist = shareDetailsFeignproxy.findAll(token);
	...
	public StockDetails getStockName(@RequestHeader("Authorization") String token,@PathVariable(value = "shareName") String shareName) {
		return shareDetailsFeignproxy.finddailyShareByName(token,shareName);
	}

	@GetMapping("/mutualfunds")
	public List<MutualFundDetails> getAllMutualFunds(@RequestHeader("Authorization") String token) {
		return mutualFundDetailsFeignProxy.getAllMututalFunds(token);
	}
